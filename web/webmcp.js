(() => {
  ...
  const changes = object({
    ...
    edges: rows({id: string, from: string, to: string}),
  ...
  const tools = [
    ...
    ['validate-architecture', 'Read-only lint for disconnected, empty, duplicate and deeply nested cards. It never fixes or deletes.', object({maxDepth: {...integer, maximum: 128, default: 8}, offset: integer, limit: {type: 'integer', minimum: 1, maximum: 500, default: 100}}), true],
    ...
    ['apply-changes', 'Atomic sparse ID upserts. Use expectedRevision for concurrency; replace=true replaces the board. Deletions reparent children/remove attached arrows. IDs and references must stay valid.',
